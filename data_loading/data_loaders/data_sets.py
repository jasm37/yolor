...
def letterbox(
    ...
    :param im: Image to apply letterbox
    :param img_size: image side size to be used as output shape (img_size, img_size)
    :param stride: stride to be used
    :param new_shape: letterbox output shape
    :param color: color to fill letterbox
    :param auto: auto: Auto apply to minimum rectangular letterbox
    ...
    :param scale_fill: stretch image size
    :param scale_up: scale up image. If False, scale down only
    :return: letter-boxed image,
...
def _grep_all_images(path: Union[str, List[str]]) -> List[str]:
    """
    Grep all image files in the path
    :param path: Root directory of the images
    :return: Every image file paths in the given path
    """
    try:
...
class LoadImages(Dataset):
    def __init__(
            ...
            image_files: Optional[Union[np.ndarray, List[np.ndarray]]] = None,
            img_shapes: Optional[Union[np.ndarray, List[np.ndarray]]] = None
    ) -> None:
        """
        Initialize LoadImages instance
        :param path: image root directory
        :param img_size: minimum width or height size
        :param batch_size: batch size to use in iterator
        :param rect: whether to use rectangular image
        :param cached_images: use caching images. If None then caching will not be used
        ...
        :param n_skip: Skip n images per one image. Ex)
                       If we have 1024 images and n_skip is 1, then total 512 images will be used.
        :param prefix: logging prefix message
        :param preprocess: preprocess function which F(x: np.ndarray) -> np.ndarray
        :param augmentation: augmentation F(x: np.ndarray) -> (np.ndarray)
        :param image_files: optional input image files.
                            If not given then these will be fetched from the given input file if possible
        :param img_shapes: optional input image shape
                           If not given then these will be fetched from the given input file if possible
        """
        self.stride = stride
        ...
        self.cache_images = cached_images
        self.n_skip = n_skip

        # Get image paths
        self.img_files = image_files if image_files is not None else _grep_all_images(path)

        # Skip n step if given.
        if n_skip > 0:
            self.img_files = [self.img_files[i] for i in range(0, len(self.img_files), n_skip)]
        ...
        self.shapes = img_shapes if img_shapes is not None else self._get_shapes()
        ...
        if cached_images and cached_images.endswith("disk"):
        ...
        if cached_images and not cached_images.startswith("dynamic"):
            ...
            for i, y in pbar:
                if cached_images == "disk":
                    if not self.img_npy[i].exists():
                        np.save(self.img_npy[i].as_posix(), y[0])
                    ...
                    (
                        ...
                        self.img_hw0[i],
                        self.img_hw[i],
                    ) = y  # im, hw_orig, hw_resized = load_image(self, i)
                    gb += self.imgs[i].nbytes  # noqa: imgs[i] is a numpy array
                pbar.desc = f"{prefix}Caching images ({gb / 1E9:.1f}GB {cached_images})"
            pbar.close()

    def shuffle(self) -> None:
        """Shufle image indices for random image order"""
        indices = np.arange(0, self.n_img)
        random.shuffle(indices)  # noqa
    ...
    def _get_shapes(self) -> np.ndarray:
        def __get_img_shape(_path: str) -> Tuple[int, int]:
            ...
            try:
                ...
                try:
                    _img_exif = _image._getexif()  # noqa: only way to get img type
    ...
    def _get_batch_shapes(self) -> Tuple[np.ndarray, np.ndarray]:
        """Get batch shape for rectangular data loading"""
        ...
        batch_shapes = (
                np.ceil(np.array(shapes) * self.img_size / self.stride + self.pad).astype("int") * self.stride
        )

        return batch_shapes, indices

    def _load_image(self, index: int) \
            -> Tuple[Optional[np.ndarray], Tuple[int, int], Tuple[int, int]]:
        """
        Load image and sizes given its index number
        :param index: image index to be loaded
        :return: OpenCv Image(NumPy)
                 image original size
                 resized image size
        """
        ...
        if im is None:  # not cached in ram
            ...
            if npy and npy.exists():  # load npy
                try:
                    im = np.load(npy)  # noqa: Paths are accepted
                except ValueError:
                    logger.warn(
            ...
            return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # im, hw_original, hw_resized
...
class LoadImagesAndLabels(LoadImages):  # for training/testing
    def __init__(
        ...
        :param path: Root directory of images. The path must include `images` directory in the middle.
        ...
        :param stride: Stride value
        :param pad: pad size for rectangular image. This applies only when rect is True
        :param n_skip: Skip n images per one image. Ex) If we have 1024 images and n_skip is 1,
                       then total 512 images will be used.
    ...
    def _load_mosaic(self, index: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, (img, _, (h, w)) in enumerate(loaded_imgs):
            ...
            mosaic_img[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # noqa
            pad_w = x1a - x1b
            pad_h = y1a - y1b
    ...
    def _cache_labels(self, path: str = "labels.cache") -> Dict:
        ...
        def _get_label(_img_path: str, _label_path: str) -> Dict[str, Any]:
            """
            Get labels information as a dictionary
            :param _img_path: resp. image file path
            :param _label_path: resp. label file path
